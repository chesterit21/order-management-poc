...
4.​ Tim DevOps sulit melakukan tracing error karena minimnya logging.
...
1. Authentication & Authorization
   Multi-role: Buyer, SellerAdmin, SellerOperator, ApplicationAdmin, DevOps.
   Login endpoint mengembalikan JWT token.
2. Store Management
   User dapat membuka toko (menjadi SellerAdmin / owner toko).
   SellerAdmin dapat membuat dan mengelola store operator.
   Setiap produk dimiliki oleh suatu store.
   Order di-scope ke store tertentu.
3. Order Management
   Create Order — Buyer membuat order yang berisi: customerId, storeId, list of items
   (productId, quantity), shippingAddress.
   Get Order — Ambil detail order berdasarkan ID.
   List Orders — List order dengan filter (status, customerId, date range, storeId) dan
   pagination.
   ...
   Cancel Order — Cancel order. Hanya bisa dilakukan jika status masih Pending atau
   Confirmed. Saat cancel, stock harus dikembalikan.
4. Backoffice Module
   Backoffice order management — list, detail, update status, cancel order.
   Backoffice product management — CRUD product, set status aktif/non-aktif, upload
   gambar produk.
   Backoffice dashboard — ringkasan statistik toko (total/active/inactive/low-stock products,
   order counts per status, today's revenue).
5. File Upload
   Upload gambar produk (maks 5 MB) melalui endpoint backoffice.
   File disimpan secara lokal di wwwroot/uploads/products.
6. Stock Management (penting untuk testing concurrency)
   Karena ini prototype, tidak perlu service inventory terpisah. Cukup buat tabel Products
   sederhana dengan kolom Id, StoreId, Name, StockQuantity, Price, RowVersion, dll.
   Aturan:
   ...
   SellerAdmin / SellerOperator dapat melakukan stock adjustment (increase / decrease)
   beserta reason.
...
5.​ Logging
   ...
   log.
6.​ Persistensi Data
   PostgreSQL dengan justifikasi: row-level locking (FOR UPDATE), unique constraint,
   check constraint (last defense stok tidak negatif), sequence (order number generation),
   JSONB (activity_logs metadata).
   Sertakan migration atau script schema.
7.​ Testing
...
8.​ Configuration
   Gunakan appsettings.json atau .env dengan key utama: Database:ConnectionString
   dan Jwt:Secret.
---